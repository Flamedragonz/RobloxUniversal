--[[
    ...
    Fixes:
    - pcall fallback syntax
    - Full cleanup in UI.Destroy()
    - Connection leak prevention
    - Background thread bound to GUI instance
    - Offset update throttled
    - Debounce on heavy buttons
    - Safe State.Mouse access
    - Atomic Config.PartName update
    - Explorer cleanup covers all instance types
...
local function clearUIElements()
    for key, _ in pairs(State.UIElements) do
        State.UIElements[key] = nil
    end
end
...
function UI.Create()
    ...
    gui.Name = "TCP_VapeStyle"
    gui.ResetOnSpawn = false
    gui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    gui.DisplayOrder = 999
    gui.IgnoreGuiInset = true

    -- FIX #1: proper pcall fallback
    ...
    -- Keep a local reference to detect stale GUI
    local thisGui = gui

    -- ===== MAIN FRAME =====
    local mf = Instance.new("Frame")
    mf.Name = "MainFrame"
    mf.Size = UDim2.new(0, 440, 0, 620)
    mf.Position = UDim2.new(0.5, -220, 0.5, -310)
    mf.BackgroundColor3 = C.Background
    mf.BorderSizePixel = 0
    mf.Active = true
    mf.Draggable = true -- NOTE: deprecated but widely supported in executors
    mf.ClipsDescendants = true
    mf.Parent = gui
    Utils.CreateCorner(mf, 12)
    Utils.CreateStroke(mf, Color3.fromRGB(45, 45, 55), 1, 0.3)
    Utils.CreateShadow(mf)
    ...
    hdr.Parent = mf
    Utils.CreateCorner(hdr, 12)

    local ha = Instance.new("Frame")
    ha.Size = UDim2.new(1, -20, 0, 2)
    ha.Position = UDim2.new(0, 10, 1, -1)
    ha.BorderSizePixel = 0
    ha.Parent = hdr
    Utils.CreateCorner(ha, 1)
    Utils.CreateGradient(ha, C.Accent, C.AccentGlow, 0)
    ...
    logo.TextSize = 14
    logo.Parent = hdr
    Utils.CreateCorner(logo, 7)

    local ttl = Instance.new("TextLabel")
    ttl.Size = UDim2.new(1, -60, 1, 0)
    ttl.Position = UDim2.new(0, 48, 0, 0)
    ttl.BackgroundTransparency = 1
    ttl.Text = "Teleport Control"
    ttl.TextColor3 = C.TextPrimary
    ...
    pinned.Size = UDim2.new(1, -20, 0, 28)
    pinned.Position = UDim2.new(0, 10, 0, 52)
    pinned.BackgroundColor3 = C.Surface
    pinned.BackgroundTransparency = 0.5
    pinned.BorderSizePixel = 0
    pinned.Parent = mf
    Utils.CreateCorner(pinned, 6)

    local pdot = Instance.new("Frame")
    pdot.Size = UDim2.new(0, 8, 0, 8)
    pdot.Position = UDim2.new(0, 10, 0.5, -4)
    pdot.BackgroundColor3 = C.Success
    pdot.BorderSizePixel = 0
    pdot.Parent = pinned
    Utils.CreateCorner(pdot, 4)

    local ptxt = Instance.new("TextLabel")
    ptxt.Size = UDim2.new(1, -26, 1, 0)
    ptxt.Position = UDim2.new(0, 24, 0, 0)
    ptxt.BackgroundTransparency = 1
    ptxt.Text = "● 0/50"
    ptxt.TextColor3 = C.TextPrimary
    ptxt.Font = Enum.Font.GothamBold
    ptxt.TextSize = 12
    ptxt.TextXAlignment = Enum.TextXAlignment.Left
    ptxt.Parent = pinned

    State.UIElements.PinnedDot = pdot
    State.UIElements.PinnedText = ptxt
    ...
    pages.Size = UDim2.new(1, -20, 1, -220)
    pages.Position = UDim2.new(0, 10, 0, 122)
    ...
    local function makePage(name, visible)
        ...
        p.Visible = visible
        p.Parent = pages
        Utils.CreateListLayout(p, nil, 8)
        Utils.CreatePadding(p, 0, 40, 0, 0)
        return p
    end
    ...
    sc.Size = UDim2.new(1, 0, 0, 56)
    sc.BackgroundColor3 = C.Surface
    sc.LayoutOrder = 1
    sc.Parent = sp
    Utils.CreateCorner(sc, 10)

    local ss = Utils.CreateStroke(sc, C.Success, 1, 0.6)

    local sd = Instance.new("Frame")
    sd.Size = UDim2.new(0, 10, 0, 10)
    sd.Position = UDim2.new(0, 14, 0.5, -5)
    sd.BackgroundColor3 = C.Success
    sd.BorderSizePixel = 0
    sd.Parent = sc
    Utils.CreateCorner(sd, 5)

    local sl = Instance.new("TextLabel")
    sl.Size = UDim2.new(0.5, -30, 1, 0)
    sl.Position = UDim2.new(0, 32, 0, 0)
    ...
    ssub.Size = UDim2.new(0.5, -14, 1, 0)
    ssub.Position = UDim2.new(0.5, 0, 0, 0)
    ssub.BackgroundTransparency = 1
    ssub.Text = "K to start"
    ssub.TextColor3 = C.TextSecondary
    ssub.Font = Enum.Font.GothamMedium
    ssub.TextSize = 11
    ssub.TextXAlignment = Enum.TextXAlignment.Right
    ssub.Parent = sc

    State.UIElements.StatusCard   = sc
    State.UIElements.StatusStroke = ss
    State.UIElements.StatusDot    = sd
    State.UIElements.StatusLabel  = sl
    State.UIElements.StatusSub   = ssub
    ...
    State.UIElements.Folder  = Comp.CreateInfoRow(sp, "📂", "Folder",
        Config.FolderPath == "" and "Workspace" or Config.FolderPath, C.TextSecondary, 12)
    State.UIElements.Filter  = Comp.CreateInfoRow(sp, "🔍", "Filter",
        Config.PartName == "" and "ALL" or Config.PartName, C.TextSecondary, 13)
    ...
    ih.Size = UDim2.new(1, 0, 0, 30)
    ih.BackgroundColor3 = C.Surface
    ih.BackgroundTransparency = 0.5
    ih.LayoutOrder = 21
    ih.Parent = sp
    Utils.CreateCorner(ih, 8)

    local iht = Instance.new("TextLabel")
    iht.Size = UDim2.new(1, -12, 1, 0)
    iht.Position = UDim2.new(0, 6, 0, 0)
    iht.BackgroundTransparency = 1
    iht.RichText = true
    iht.Text = '💡 <font color="#FFE632">Yellow</font> = not confirmed. Press <font color="#5A50DC">Enter ↵</font>'
    iht.TextColor3 = C.TextSecondary
    iht.Font = Enum.Font.GothamMedium
    iht.TextSize = 11
    iht.TextXAlignment = Enum.TextXAlignment.Left
    iht.Parent = ih
    ...
    Comp.CreateInput(stp, "📂 Folder Path", "e.g. ItemDebris", Config.FolderPath, function(t)
        Config.FolderPath = t
        if State.UIElements.Folder then
            State.UIElements.Folder.Text = t == "" and "Workspace" or t
        end
    end, 2)

    -- Auto-detect button (with debounce)
    local autoDetectBusy = false
    Comp.CreateButton(stp, "🔍 Auto-detect Folders", C.AccentDark, function()
        if autoDetectBusy then return end
        if not Scanner then Notify.Send("❌ Scanner N/A", C.Danger); return end
        autoDetectBusy = true
        local ok, err = pcall(function()
            local folders = Scanner.FindFoldersWithParts(3)
            if #folders == 0 then
                Notify.Send("🔍 No folders found", C.TextSecondary)
                return
            end
            for i = 1, math.min(3, #folders) do
                Notify.Send("📂 " .. folders[i].path .. " (" .. folders[i].partCount .. ")", C.Info, 4)
            end
        ...
        if not ok then
            Notify.Send("❌ Scan error", C.Danger)
        end
        autoDetectBusy = false
    end, 3)

    -- FIX #12: atomic Config.PartName update
    Comp.CreateInput(stp, "🔍 Part Filter", "e.g. Gold (empty=ALL)", Config.PartName, function(t)
        if t:find(",") then
            ...
            Config.PartNames = names
            Config.PartName = "" -- multi-filter mode
        else
            Config.PartNames = {}
            Config.PartName = t
        end
        if State.UIElements.Filter then
            State.UIElements.Filter.Text = (t == "") and "ALL" or t
        end
    end, 4)
    ...
    local ctg = Instance.new("Frame")
    ctg.Size = UDim2.new(1, 0, 0, 0)
    ctg.AutomaticSize = Enum.AutomaticSize.Y
    ctg.BackgroundTransparency = 1
    ctg.Visible = Config.TargetMode ~= "Mouse"
    ctg.LayoutOrder = 8
    ctg.Parent = stp
    Utils.CreateListLayout(ctg, nil, 8)
    State.UIElements.CustomTargetGroup = ctg

    Comp.CreateToggle(stp, "🖱️ Mouse Target", Config.TargetMode == "Mouse", function(v)
        Config.TargetMode = v and "Mouse" or "CustomPart"
        ctg.Visible = not v
        if State.UIElements.Target then
    ...
    local cti = Comp.CreateInput(ctg, "📍 Target Name", "Part name", Config.TargetPartName, function(t)
        Config.TargetPartName = t
        if t ~= "" then
            State.CustomTargetPart = nil
            Engine.UpdateSelectionBox(nil)
        end
        if State.UIElements.Target then
            State.UIElements.Target.Text = t == "" and "Player" or t
        end
    end)
    ...
    local selBtn = Comp.CreateButton(ctg, "👆 Click to Select", Color3.fromRGB(50, 80, 140), function(btn)
        ...
        Utils.Tween(btn, {
            BackgroundColor3 = State.IsSelectingTarget and C.Danger or Color3.fromRGB(50, 80, 140)
        }, 0.2)
    end)
    ...
    Comp.CreateInput(advC, "⛓️ Spacing", "2", tostring(Config.Spacing), function(t)
        Config.Spacing = tonumber(t) or 2
    end, 2)

    local ofi = Comp.CreateOffsetInputs(advC, Config.Offset, nil, 3)

    Comp.CreateToggle(advC, "⚓ Anchor on Release", Config.AnchorOnFinish, function(v)
        Config.AnchorOnFinish = v
    end, 4)
    ...
    Comp.CreateToggle(advC, "💫 Smooth Teleport", Config.SmoothTeleport, function(v)
        ...
        if State.UIElements.TPType then
            State.UIElements.TPType.Text = v and "Smooth" or "Instant"
        end
        Engine.ReprepareParts()
    end, 6)
    Comp.CreateInput(advC, "💨 Speed", "25", tostring(Config.SmoothSpeed), function(t)
        Config.SmoothSpeed = tonumber(t) or 25
    end, 7)

    -- Visuals
    Comp.CreateSection(advC, "VISUALS", 8)
    Comp.CreateToggle(advC, "✨ ESP Highlight", Config.ESPEnabled, function(v)
        Engine.ESP.Toggle(v)
    end, 9)
    Comp.CreateToggle(advC, "🔊 Sound Effects", Config.SoundEnabled, function(v)
    ...
    if Presets then
        ...
        Comp.CreateButton(advC, "💾 Save Current Settings", C.AccentDark, function()
            ...
            if name and name ~= "" then
                ...
                Notify.Send("⚠️ Enter name first", C.Warning)
            end
        end, 15)
            ...
            if name and name ~= "" then
                ...
                if #names > 0 then
                    for _, n in ipairs(names) do
                        Notify.Send("📂 " .. n, C.Info, 3)
                    end
                else
                    Notify.Send("No saved presets", C.TextSecondary)
                end
            end
        end, 16)
    end

    -- FIX #5 & #6: Disconnect old, throttle offset update
    if State.Connections["OffsetUpdate"] then
        State.Connections["OffsetUpdate"]:Disconnect()
        State.Connections["OffsetUpdate"] = nil
    end

    local offsetAccum = 0
    State.Connections["OffsetUpdate"] = RunService.Heartbeat:Connect(function(dt)
        -- Stop if GUI was destroyed
        if not thisGui or not thisGui.Parent then
            if State.Connections["OffsetUpdate"] then
                State.Connections["OffsetUpdate"]:Disconnect()
                State.Connections["OffsetUpdate"] = nil
            end
        ...
        -- Throttle: 4 times per second
        offsetAccum = offsetAccum + dt
        if offsetAccum >= 0.25 then
            offsetAccum = 0
            if ofi then
                Config.Offset = ofi.GetOffset()
            end
        end
    end)
    ...
    Comp.CreateButton(ep, "🔍 Scan Workspace", C.Accent, function()
        ...
        pcall(function()
            -- FIX #11: Clear old results — remove ALL children with LayoutOrder >= 10
            for _, c in pairs(ep:GetChildren()) do
                if c:IsA("GuiObject") and c.LayoutOrder >= 10 then
                    c:Destroy()
                end
            end
            ...
            for i, f in ipairs(folders) do
                ...
                row.LayoutOrder = 13 + i
                row.Parent = ep
                Utils.CreateCorner(row, 8)
                ...
                nameLabel.Text = depth .. "📂 " .. f.name .. " (" .. f.partCount .. ")"
    ...
    -- Show unique part names (with debounce)
    local partNamesBusy = false
    Comp.CreateButton(ep, "📋 Show Part Names in Current Folder", C.AccentDark, function()
        if not Scanner then return end
        if partNamesBusy then return end
        partNamesBusy = true
        ...
        pcall(function()
            ...
            if count == 0 then
                Notify.Send("No parts in folder", C.TextSecondary)
            end
        end)
    ...
    for i, name in ipairs(tabNames) do
        ...
        btn.MouseButton1Click:Connect(function()
            for j, pg in ipairs(pagesList) do
                pg.Visible = (j == i)
    ...
    ft.Size = UDim2.new(1, -20, 0, 88)
    ft.Position = UDim2.new(0, 10, 1, -96)
    ...
    tb.BackgroundColor3 = C.Warning
    tb.Text = "⏸️ DEACTIVATE"
    ...
    tb.Parent = ft
    Utils.CreateCorner(tb, 10)

    tb.MouseButton1Click:Connect(function()
        Utils.AnimateClick(tb)
        -- FIX #13: safe mouse access
        if State.Mouse then
            Utils.Ripple(tb, State.Mouse.X, State.Mouse.Y)
        end

        State.IsActive = not State.IsActive
        if State.IsActive then
            tb.Text = "⏸️ DEACTIVATE"
            Utils.Tween(tb, {BackgroundColor3 = C.Warning}, 0.2)
            Notify.Send("▶️ Activated", C.Success)
        else
            tb.Text = "▶️ ACTIVATE"
            Utils.Tween(tb, {BackgroundColor3 = C.Success}, 0.2)
    ...
    sr.Size = UDim2.new(1, 0, 0, 36)
    sr.Position = UDim2.new(0, 0, 0, 46)
    ...
    local function fBtn(text, color, sz, cb)
        ...
        b.Parent = sr
        Utils.CreateCorner(b, 8)
        Utils.AddHover(b, 0.15, 0)
        b.MouseButton1Click:Connect(function()
            Utils.AnimateClick(b)
            cb(b)
        end)
        return b
    end
    ...
    fBtn("📌 Mini", C.SurfaceLight, 0.25, function()
        ...
        local miniPanel = gui:FindFirstChild("MiniPanel")
        if miniPanel then
    ...
    -- Close — FIX #2 & #9: full cleanup, no race condition
    fBtn("✕", C.Danger, 0.25, function()
        ...
        if Presets then
            pcall(function() Presets.AutoSave() end)
        end
        ...
        -- Nil out State.GUI before the delayed destroy so a re-create won't collide
        State.GUI = nil
        clearUIElements()
    ...
    mp.Name = "MiniPanel"
    mp.Size = UDim2.new(0, 200, 0, 36)
    mp.Position = UDim2.new(0.5, -100, 0, 10)
    mp.BackgroundColor3 = C.Background
    mp.BackgroundTransparency = 0.1
    mp.BorderSizePixel = 0
    mp.Visible = false
    mp.Active = true
    mp.Draggable = true -- NOTE: deprecated but widely supported in executors
    mp.Parent = gui
    Utils.CreateCorner(mp, 18)
    Utils.CreateStroke(mp, C.Accent, 1, 0.5)

    local md = Instance.new("Frame")
    md.Size = UDim2.new(0, 10, 0, 10)
    md.Position = UDim2.new(0, 12, 0.5, -5)
    md.BackgroundColor3 = C.Success
    md.BorderSizePixel = 0
    md.Parent = mp
    Utils.CreateCorner(md, 5)

    local mc = Instance.new("TextLabel")
    mc.Size = UDim2.new(0, 80, 1, 0)
    mc.Position = UDim2.new(0, 28, 0, 0)
    mc.BackgroundTransparency = 1
    mc.Text = "0/50"
    mc.TextColor3 = C.TextPrimary
    ...
    ml.Size = UDim2.new(0, 24, 1, 0)
    ml.Position = UDim2.new(0, 110, 0, 0)
    ml.BackgroundTransparency = 1
    ml.Text = "⏸"
    ml.TextColor3 = C.TextSecondary
    ml.Font = Enum.Font.GothamBold
    ml.TextSize = 16
    ml.Parent = mp

    local me = Instance.new("TextButton")
    me.Size = UDim2.new(0, 44, 0, 24)
    me.Position = UDim2.new(1, -52, 0.5, -12)
    me.BackgroundColor3 = C.Accent
    me.BackgroundTransparency = 0.5
    me.Text = "▼"
    me.TextColor3 = C.TextPrimary
    me.Font = Enum.Font.GothamBold
    me.TextSize = 12
    me.Parent = mp
    Utils.CreateCorner(me, 6)
    ...
    State.UIElements.MiniDot     = md
    State.UIElements.MiniCounter = mc
    State.UIElements.MiniLoop    = ml

    -- FIX #4: Mini panel updater bound to THIS gui instance
    task.spawn(function()
        while State.IsRunning and thisGui and thisGui.Parent do
            if mp and mp.Parent and mp.Visible then
